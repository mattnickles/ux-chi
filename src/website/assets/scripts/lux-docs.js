...
function enableCopyToClipboardFeature (preElem) {
  ...
  copyButton.setAttribute('class', 'clipboard__button lux-button -sm -flat');
...
onLoad(() => {
  ...
  var examples = document.querySelectorAll('.lux-example');
  ...
  Array.prototype.forEach.call(examples, function(example) {
    ...
    const htmlItem = example.querySelector('.lux-example__html');
    const codeItem = example.querySelector('.lux-example__code');
 ...
 // Polyfill element.closest IE9+
  ...
  lux.dropdown(dropdownButton);
  lux.dropdown(document.getElementById('support'));
  ...
  if (urlHash) {
    ...
    Array.prototype.forEach.call(document.querySelectorAll('article.docs-article > section.lux-grid__container > .lux-tabs-panel'), function (tabContent) {
      if (document.querySelector(`article ${urlHash}`)) {
