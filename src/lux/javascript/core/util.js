...
export class Util {
  ...
  static getData (elem, key) {
    return elem[lux.expando] && elem[lux.expando][key];
  }

  static isSetData (elem, key) {
    return elem[lux.expando] && elem[lux.expando].hasOwnProperty(key);
  }

  static removeData (elem, key) {
    delete elem[lux.expando][key];
    if (Object.getOwnPropertyNames(elem[lux.expando]).length === 0) {
  ...
  static registerComponent (type, elem, obj) {
    ...
    if (!lux.registeredComponents[type]) {
      lux.registeredComponents[type] = {};
    }
    lux.registeredComponents[type][index] = obj;
  }

  static unregisterComponent (type, elem) {
    let index = Util.getData(elem, type);
    if (lux.registeredComponents[type]) {
      delete lux.registeredComponents[type][index];
    }
  ...
  static getRegisteredComponent (type, elem) {
    let index = Util.getData(elem, type);
    if (index || index === 0) {
      return lux.registeredComponents[type] &&
        lux.registeredComponents[type][index];
    } else {
      return false;
    }
  }
