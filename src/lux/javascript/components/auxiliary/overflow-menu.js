...
class OverflowMenu {
  ...
  _createOverflowTab () {
    ...
    Util.addClass(this._overflowTab, 'lux-dropdown');
    this._overflowTab.innerHTML =
      '<a href="#" class="lux-dropdown__trigger" data-position="bottom-end"> ' +
      this._config.overflowMenuLabel + '</a>' +
      '<div class="lux-dropdown__menu"></div>';
    this._overflowTabMenu =
      this._overflowTab.getElementsByClassName('lux-dropdown__menu')[0];
    this._overflowTabTrigger =
      this._overflowTab.getElementsByClassName('lux-dropdown__trigger')[0];
  ...
  _moveTabToMoreContainer (tab, width) {
    ...
    while (tab.childNodes.length) {
      ...
      if (child.nodeName === 'A') {
        Util.addClass(child, 'lux-dropdown__menu-item');
      }
      if (Util.hasClass(child,'lux-dropdown__trigger')) {
  ...
  _moveTabFromMoreContainer (tab) {
    ...
    while (tab.childNodes.length) {
      const child = tab.childNodes[0];
      if (child.nodeName === 'A') {
        Util.removeClass(child, 'lux-dropdown__menu-item');
      }
      if (Util.hasClass(child,'lux-dropdown__trigger')) {
