...
const COMPONENT_SELECTOR = '.lux-sidenav';
const COMPONENT_TYPE = "sidenav";
const DRAWER_CLASS = "lux-drawer";
const LINKLIST_CLASS = 'lux-sidenav__list';
const SIDENAV_CONTENT_CLASS = 'lux-sidenav__content';
const SIDENAV_DRAWERS_CLASS = 'lux-sidenav__drawers';
const SIDENAV_TITLE_CLASS = 'lux-sidenav__title';
const LUX_DRAWER_CONTENT = 'lux-drawer__content';
const DRAWER_LINKLIST_CLASS = "lux-sidenav__drawer-list";
const DRAWER_ITEM_LIST_CLASS = "lux-sidenav__drawer-item-list";
const DRAWER_ITEM_TAB_CLASS = "lux-sidenav__drawer-item-tab";
const DRAWER_SUBITEM_TRIGGER_CLASS = "lux-drawer__subitem-list-trigger";
...
class Sidenav extends Component {
  ...
  constructor(elem, config) {
    ...
    if (this._config.animated) {
      Util.checkAddClass(this._elem, lux.classes.ANIMATED);
    }
  ...
  _resetDrawerAnimation() {
    if (this._config.animated) {
      this._drawers.forEach(function(drawer) {
        drawer._drawerElem.classList.add(lux.classes.ANIMATED);
      });
    }
  }
      ...
      const disableAnimation = (drawer) => {
        this._drawers.forEach((otherDrawer) => {
          if (otherDrawer !== drawer) {
            otherDrawer._drawerElem.classList.remove(lux.classes.ANIMATED);
          }
        });
      };
      ...
      this._drawers.forEach((drawer) => {
        this._addEventHandler(
          ...
          () => {
            ...
            if (selectedMenuItem) {
              Util.removeClass(selectedMenuItem, lux.classes.ACTIVE);
            }
  ...
  _initSubtabTriggers() {
    const secondLevelItems = this._elem.querySelectorAll(`.${DRAWER_CLASS} .${LUX_DRAWER_CONTENT} ul.${DRAWER_LINKLIST_CLASS} li`);
  ...
  singleLevelMenuItems() {
    Array.prototype.forEach.call(
      ...
      (singleLevelMenuItem) => {
        ...
        menuElementLink[0] === '#' ?
        this._elem.querySelector(`.lux-drawer${menuElementLink}`) :
        null;
        ...
        if (!menuElementDrawer) {
            ...
            this._addEventHandler(
              ...
              (e) => {
                ...
                if (menuItemAnchorToSelect) {
                  Util.addClass(menuItemAnchorToSelect, lux.classes.ACTIVE);
                }
                if (activeMenuItem && activeMenuItem !== e.target) {
                  Util.addClass(activeMenuItem.querySelector('a'), lux.classes.UNSELECTED);
                }
            });
          }
          ...
          this._addEventHandler(
            ...
            if (activeMenuItem) {
              Util.removeClass(activeMenuItem, lux.classes.ACTIVE);
              Array.prototype.forEach.call(
                this._elem
                .querySelectorAll(
                  `.lux-drawer .${LUX_DRAWER_CONTENT} .${DRAWER_LINKLIST_CLASS} li.${lux.classes.ACTIVE},
                  .lux-drawer .${LUX_DRAWER_CONTENT} .${DRAWER_LINKLIST_CLASS} li a.${DRAWER_ITEM_TAB_CLASS}.${lux.classes.ACTIVE}`
                ),
                (activeDrawerElement) => {
                  Util.removeClass(activeDrawerElement, lux.classes.ACTIVE);
                  if (Util.hasClass(activeDrawerElement, lux.classes.EXPANDED)) {
                    Util.removeClass(activeDrawerElement, lux.classes.EXPANDED);
                  }
                });
              this.hideAll();
            }
  ...
  getActiveMenuItem() {
    return this._elem.querySelector(
      'ul.' + LINKLIST_CLASS + '>li.' + lux.classes.ACTIVE
    );
  }

  getSelectedMenuItem() {
    return this._elem.querySelector(
      'ul.' + LINKLIST_CLASS + '>li a.' + lux.classes.ACTIVE
    );
  }

  getActiveDrawerItemSubtab() {
    return this._elem
    .querySelector(`.lux-drawer .${DRAWER_ITEM_LIST_CLASS} ul li a.${lux.classes.ACTIVE}`);
  }

  getDrawerActiveMenuItem() {
    return this._elem.querySelector(
      'ul.' + DRAWER_LINKLIST_CLASS + '>li.' + lux.classes.ACTIVE
    );
  }

  getDrawerExpandedMenuItem() {
    return this._elem.querySelector(
      'ul.' + DRAWER_LINKLIST_CLASS + '>li.' + lux.classes.EXPANDED
    );
  }
  ...
  activateMenuItem(menuItem) {
    ...
    const currentlySelectedMenuItem = menuItem.parentNode.querySelector(`li a.${lux.classes.UNSELECTED}`);

    if (menuItem === currentlyActiveMenuItem) {
      return;
    }

    Util.checkAddClass(menuItem, lux.classes.ACTIVE);
    Util.checkRemoveClass(currentlySelectedMenuItem, lux.classes.UNSELECTED);
    Util.checkRemoveClass(currentlyActiveMenuItem, lux.classes.ACTIVE);
  }
  ...
  activateDrawerMenuItem(menuItem) {
    ...
    if (currentlyActiveDrawerMenuItem) {
      ...
      if (!menuItem.querySelector(`.${DRAWER_ITEM_LIST_CLASS}`)) {
        Util.checkRemoveClass(currentlyActiveDrawerItemSubtab, lux.classes.ACTIVE);
        Util.checkRemoveClass(currentlyActiveDrawerMenuItem, lux.classes.ACTIVE);
      }

      if (currentlyActiveItemList) {
        Util.checkRemoveClass(currentlyActiveDrawerMenuItem, lux.classes.EXPANDED);
        Util.checkRemoveClass(currentlyActiveItemList, lux.classes.TRANSITIONING);
  ...
  menuItemRemoveUnselected() {
    ...
    if (currentlyActiveMenuItem) {
      const unselectedMenuItem = currentlyActiveMenuItem.querySelector(`a.${lux.classes.UNSELECTED}`);

      Util.checkRemoveClass(unselectedMenuItem, lux.classes.UNSELECTED);
    }
  }
  ...
  toggleDrawerItemList(drawerMenuItem) {
    ...
    const expandedItem = drawerMenuItem.parentNode.querySelector(`.${lux.classes.EXPANDED}`);
    ...
    if (!Util.checkHasClass(drawerMenuItem, lux.classes.EXPANDED)) {
      this._menuItemAnimation = Util.threeStepsAnimation(
        function() {
          ...
          Util.checkAddClass(drawerMenuItem, lux.classes.EXPANDED);
        }, function() {
      ...
      this._menuItemAnimation = Util.threeStepsAnimation(
        function() {
          ...
          drawerMenuItemList.style.height = '0px';
          Util.checkRemoveClass(drawerMenuItem, lux.classes.EXPANDED);
        }, function() {
          Util.checkRemoveClass(drawerMenuItemList, lux.classes.TRANSITIONING);
  ...
  hideAll() {
    ...
    if (this._config.openOnHover) {
      Util.checkRemoveClass(selectedMenuItem, lux.classes.ACTIVE);
    }
  }
  ...
  _handlerClickOnDrawer(e) {
    ...
    for (let cur = e.target; cur && cur !== this._drawersContainer; cur = cur.parentNode) {
      if (Util.checkHasClass(cur, DRAWER_CLASS)) {
        ...
        !Util.checkHasClass(cur, lux.classes.CLOSE)
      ) {
        activator = cur;
      ...
      if (drawerMenuItem) {
        ...
        if (drawerMenuItemList) {
          ...
          if (Util.checkHasClass(activator, DRAWER_ITEM_TAB_CLASS)) {
            ...
            this.drawerMenuItemRemoveUnselected();
            Util.checkRemoveClass(this.getDrawerActiveMenuItem(), lux.classes.ACTIVE);
            Util.checkAddClass(activator, lux.classes.ACTIVE);
  ...
  _handlerDrawerShown(e) {
    ...
    if (menuItemLink) {
      ...
      const activeItemLink = menuItemLink.parentNode.parentNode.querySelector(`li.${lux.classes.ACTIVE} a`);

      if (activeItemLink) {
        if (!Util.checkHasClass(menuItemLink.parentNode, lux.classes.ACTIVE)) {
          Util.checkAddClass(activeItemLink, lux.classes.UNSELECTED);
        } else if (Util.checkHasClass(activeItemLink, lux.classes.UNSELECTED)) {
          Util.checkRemoveClass(activeItemLink, lux.classes.UNSELECTED);
        }
      }
    }
  }
  ...
  _handlerDrawerHide() {
    const selectedSingleLevel = this._elem.querySelector(`nav ul li a.${lux.classes.ACTIVE}`);
    const allDrawersClosed = this._drawers.every(drawer => !Util.checkHasClass(drawer._elem, lux.classes.ACTIVE)) && !selectedSingleLevel;
            ...
            if (self._elem.querySelector('nav').contains(e.target)) {
              for (let cur = e.target; cur.nodeName !== 'LI'; cur = cur.parentNode) {
                if (cur.nodeName === 'A') {
                  ...
                  if (drawerToCheck) {
                    if (!drawerToCheck.classList.contains(lux.classes.ANIMATED)) {
