...
class DatePicker extends Component {
  ...
  _dayClickEventHandler (e) {
    ...
    if (
      Util.hasClass(target, 'lux-datepicker__day') &&
      !Util.hasClass(target, lux.classes.INACTIVE)
    ) {
  ...
  _initContainer() {
    ...
    this._addEventHandler(this.elems.prevButton, 'click', function(e){
      if (!Util.hasClass(e.target, lux.classes.DISABLED)) {
        self.loadPrevMonth();
      }
    });
    ...
    if (
      ...
      Util.hasClass(this._elem.parentElement, 'lux-input__wrapper')
    ) {
    ...
    this._popover = Popover.factory(parent, {
      arrow: false,
      classes: ['lux-popover__datepicker'],
  ...
  _renderEmptyCalendar() {
    ...
    calendar.setAttribute(
      'class', 'lux-datepicker ' + this._weekStartClass
    );

    calendar.innerHTML = `
    <div class="lux-datepicker__month-row">
      <div class="prev">
        <i class="lux-icon icon-chevron-left -sm"></i>
      </div>
      <div class="lux-datepicker__month"></div>
      <div class="next">
        <i class="lux-icon icon-chevron-right -sm"></i>
      </div>
    </div>
    <div class="lux-datepicker__day-names">
      ${this._renderWeekDays()}
    </div>
    <div class="lux-datepicker__days"></div>`;

    this.elems.dayList = calendar.getElementsByClassName('lux-datepicker__days')[0];
    this.elems.monthTitle = calendar.getElementsByClassName('lux-datepicker__month')[0];
  ...
  _updateCalendarView(month) {
    ...
    if (endOfPrevMonth.isBefore(dayjs(this._config.min))) {
      Util.addClass(this.elems.prevButton, lux.classes.DISABLED);
    } else {
      Util.removeClass(this.elems.prevButton, lux.classes.DISABLED);
    }
    if (startOfNextMonth.isAfter(dayjs(this._config.max))) {
      Util.addClass(this.elems.nextButton, lux.classes.DISABLED);
    } else {
      Util.removeClass(this.elems.nextButton, lux.classes.DISABLED);
    }
  ...
  _classesForDay(day) {
    const classes = ['lux-datepicker__day'];
    if (
      day.isBefore(dayjs(this._config.min)) ||
      day.isAfter(dayjs(this._config.max))
    ) {
      classes.push(lux.classes.INACTIVE);
    }
    if (day.isSame(this.date, 'day')) {
      classes.push(lux.classes.ACTIVE);
    }
