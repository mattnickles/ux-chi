...
const CLASS_DRAWER = 'lux-drawer';
const CLOSE_TRIGGER_SELECTOR = `.${CLASS_DRAWER} > .-close, .${CLASS_DRAWER} > .lux-drawer__header > .-close`;
const COMPONENT_SELECTOR = '.lux-drawer__trigger';
const COMPONENT_TYPE = 'drawer';
const DISABLE_SCROLL = '-disable-scroll';
const EVENTS = {
  show: 'lux.drawer.show',
  hide: 'lux.drawer.hide'
};
...
class Drawer extends Component {
  ...
  constructor(elem, config) {
    ...
    this._shown = Util.hasClass(elem, lux.classes.ACTIVE);
    ...
    if (this._config.animated) {
      Util.addClass(this._drawerElem, lux.classes.ANIMATED);
      if (this._backdrop) {
        Util.addClass(this._backdrop, lux.classes.ANIMATED);
      }
    }
  ...
  _locateBackdrop() {
    ...
    if (Util.hasClass(parent, 'lux-backdrop')) {
      return parent;
    } else {
      return null;
    }
    ...
    if (!this._shown) {
      ...
      if (this._config.animated) {
        this._currentThreeStepsAnimation = Util.threeStepsAnimation(
          function() {
            Util.addClass(self._drawerElem, lux.classes.TRANSITIONING);
            if (self._backdrop) {
              Util.addClass(self._backdrop, lux.classes.TRANSITIONING);
            }
          },
          function() {
            Util.addClass(self._drawerElem, lux.classes.ACTIVE);
            Util.addClass(self._elem, lux.classes.ACTIVE);
            if (self._backdrop) {
              Util.removeClass(self._backdrop, lux.classes.CLOSED);
            }
            self._shown = true;
          ...
          function() {
            Util.removeClass(self._drawerElem, lux.classes.TRANSITIONING);
            if (self._backdrop) {
              Util.removeClass(self._backdrop, lux.classes.TRANSITIONING);
            }
    ...
    if (this._shown) {
      ...
      if (this._config.animated) {
        this._currentThreeStepsAnimation = Util.threeStepsAnimation(
          ...
          function() {
            Util.removeClass(self._drawerElem, lux.classes.ACTIVE);
            Util.removeClass(self._elem, lux.classes.ACTIVE);
            if (self._backdrop) {
              Util.addClass(self._backdrop, lux.classes.CLOSED);
            }
            self._shown = false;
            self._drawerElem.dispatchEvent(
