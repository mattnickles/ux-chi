...
import { lux } from '../core/lux';
import { Drawer, DRAWER_ANIMATION_DURATION } from './drawer';
import { Dropdown } from './dropdown';

const LUX_TABS_CLASS = 'lux-tabs';
const COMPONENT_TYPE = 'mobilenav';
const DRAWER_CLASS = 'lux-drawer';
const DRAWER_CONTENT_CLASS = 'lux-drawer__content';
const DRAWER_HEADER_CLASS = 'lux-drawer__header';
const DRAWER_ITEM_LIST_ANIMATION_DURATION = 75;
const DRAWER_ITEM_LIST_CLASS = 'lux-mobile-nav__list-item-list';
const DRAWER_ITEM_TAB_CLASS = 'lux-mobile-nav-item-tab';
const DRAWER_ITEM_LIST_EXPANDED = '-expanded';
const DRAWER_NAV_LIST = 'lux-mobile-nav__list';
const DRAWER_SUBITEM_LIST_TRIGGER_CLASS = 'lux-drawer__subitem-list-trigger';
const DROPDOWN_TRIGGER = 'lux-dropdown__trigger';
const FIRST_LEVEL_DRAWER_CLASS = 'lux-mobile-nav__first-level';
const FIRST_LEVEL_DRAWER_CONTENT_CLASS = 'lux-mobile-nav__first-level-content';
const MOBILENAV_DRAWERS_CLASS = 'lux-mobile-nav__drawers';
const MOBILENAV_LIST_TITLE = 'lux-mobile-nav__list-title';
...
class MobileNav extends Component {
  ...
  _initFirstLevelDrawer() {
    ...
    const firstLevelMenuItems = this._mobileNavElement.querySelectorAll(`.${DRAWER_CLASS}.${FIRST_LEVEL_DRAWER_CLASS} .${FIRST_LEVEL_DRAWER_CONTENT_CLASS} .${DRAWER_CONTENT_CLASS} .${LUX_TABS_CLASS} li a`);
    ...
    if (firstLevelDrawer) {
      const firstLevelCloseButton = firstLevelDrawer.querySelector(`.${lux.classes.CLOSE}`);
    ...
    Array.prototype.forEach.call(
      ...
      (menuItem) => {
        ...
        const drawer = document.querySelector(`.lux-drawer${drawerId}`);

        if (drawer) {
          const returnBackButton = drawer.querySelector(`.${DRAWER_HEADER_CLASS} button.${lux.classes.BACK}`);
          ...
          this._addEventHandler(
            ...
            function(e) {
              e.preventDefault();
              Util.addClass(drawer, lux.classes.ACTIVE);
          ...
          this._addEventHandler(
            ...
            function(e) {
              e.stopPropagation();
              Util.removeClass(drawer, lux.classes.ACTIVE);
    ...
    this._addEventHandler(
      ...
      function() {
        ...
        const activeSecondLevelDrawer = self._mobileNavElement.querySelector(`.${MOBILENAV_DRAWERS_CLASS} .${DRAWER_CLASS}.${lux.classes.ACTIVE}`);

        if (activeSecondLevelDrawer) {
          setTimeout(
            function() {
              Util.removeClass(activeSecondLevelDrawer, lux.classes.ACTIVE);
  ...
  _getActiveFirstLevelItem() {
    return this._mobileNavElement.querySelector(`.${FIRST_LEVEL_DRAWER_CLASS} .${DRAWER_CONTENT_CLASS} .${LUX_TABS_CLASS} li.${lux.classes.ACTIVE}`);
  }

  _getActiveSecondLevelItem() {
    return this._mobileNavElement.querySelector(`.${MOBILENAV_DRAWERS_CLASS} .${DRAWER_CLASS} .${DRAWER_CONTENT_CLASS} .${DRAWER_NAV_LIST} li.${lux.classes.ACTIVE}`);
  }

  _getActiveSecondLevelItemSubtab() {
    return this._mobileNavElement.querySelector(`.${MOBILENAV_DRAWERS_CLASS} .${DRAWER_CLASS} .${DRAWER_CONTENT_CLASS} .${DRAWER_NAV_LIST} li.${lux.classes.ACTIVE} .${DRAWER_ITEM_LIST_CLASS} li a.${lux.classes.ACTIVE}`);
  }

  _getExpandedSecondLevelItem() {
    return this._mobileNavElement.querySelector(`.${MOBILENAV_DRAWERS_CLASS} .${DRAWER_CLASS} .${DRAWER_CONTENT_CLASS} .${DRAWER_NAV_LIST} li.${lux.classes.EXPANDED}`);
  }
  ...
  _activateFirstLevelMenuItem(menuItem) {
    ...
    if (menuItemToActivate.nodeName === 'LI') {
      if (currentlyActiveMenuItem) {
        Util.removeClass(currentlyActiveMenuItem, lux.classes.ACTIVE);
      }
      Util.addClass(menuItemToActivate, lux.classes.ACTIVE);
      this.hide();
    }

    if (!menuItemDrawer) {
      const menuItemToClose = this._drawersContainer.querySelector(`.${DRAWER_CLASS} .${DRAWER_CONTENT_CLASS} .${DRAWER_NAV_LIST} li.${lux.classes.ACTIVE}`);

      if (menuItemToClose) {
        const menuItemList = menuItemToClose.querySelector(`.${DRAWER_ITEM_LIST_CLASS}`);
        const menuItemSubItemToClose = menuItemToClose.querySelector(`.${DRAWER_ITEM_LIST_CLASS} li a.${lux.classes.ACTIVE}`);

        Util.removeClass(menuItemToClose, lux.classes.ACTIVE);

        if (menuItemList) {
          Util.removeClass(menuItemList, lux.classes.TRANSITIONING);
          Util.removeClass(menuItemToClose, lux.classes.EXPANDED);
          menuItemList.style.removeProperty('height');

          if (menuItemSubItemToClose) {
            Util.removeClass(menuItemSubItemToClose, lux.classes.ACTIVE);
          }
        }
      }
    }
  ...
  _closeTabsList(menuItem) {
    ...
    if (Util.hasClass(menuItem, lux.classes.EXPANDED)) {
      Util.removeClass(menuItem, lux.classes.EXPANDED);
    }

    if (menuItemTabs) {
      const activeSubtab = menuItemTabs.querySelector(`a.${lux.classes.ACTIVE}`);

      Util.removeClass(activeSubtab, lux.classes.ACTIVE);
      menuItemTabs.style.removeProperty('height');
    }
  }

  _activateSecondLevelMenuItem(menuItemActivator, drawerMenuItem = null, drawerId = '') {
    const firstLevelItemToDeactive = this._mobileNavElement.querySelector(`.${FIRST_LEVEL_DRAWER_CLASS} .${DRAWER_CONTENT_CLASS} ul.${LUX_TABS_CLASS} > li.${lux.classes.ACTIVE}`);
    const _removeSecondLevelActive = () => {
      ...
      if (activeSecondLevel) {
        ...
        Util.removeClass(activeSecondLevel, lux.classes.ACTIVE);
      }
    };

    if (firstLevelItemToDeactive) {
      if (drawerId !== firstLevelItemToDeactive.getAttribute('href')) {
        const firstLevelItemToActive = this._mobileNavElement.querySelector(`.${FIRST_LEVEL_DRAWER_CLASS} .${DRAWER_CONTENT_CLASS} ul.${LUX_TABS_CLASS} > li a[href='#${drawerId}']`);

        Util.removeClass(firstLevelItemToDeactive, lux.classes.ACTIVE);
        Util.addClass(firstLevelItemToActive.parentNode, lux.classes.ACTIVE);
      }
    }
    ...
    if (Util.hasClass(menuItemActivator, DRAWER_ITEM_TAB_CLASS)) {
      ...
      if (activeSecondLevelItem && !activeSecondLevelItem.contains(menuItemActivator)) {
        Util.removeClass(this._getActiveSecondLevelItem(), lux.classes.ACTIVE);
      }
      Util.addClass(menuItemActivator, lux.classes.ACTIVE);
      Util.addClass(drawerMenuItem, lux.classes.ACTIVE);
      if (currentlyActiveSecondLevelItemSubtab && currentlyActiveSecondLevelItemSubtab !== menuItemActivator) {
        Util.checkRemoveClass(currentlyActiveSecondLevelItemSubtab, lux.classes.ACTIVE);
      }
    } else {
      const menuItem = menuItemActivator.parentNode;

      _removeSecondLevelActive();
      Util.addClass(menuItem, lux.classes.ACTIVE);
    }
  }
  ...
  _handleClickOnSecondLevelDrawer(e) {
    ...
    if (Util.checkHasClass(e.target, lux.classes.CLOSE) ||
      Util.checkHasClass(e.target.parentNode, lux.classes.CLOSE) ||
      Util.checkHasClass(e.target.parentNode.parentNode, lux.classes.CLOSE)) {
      this._hideMobileNav();
    }
    ...
    for (let currentNode = e.target; currentNode && currentNode !== this._drawersContainer; currentNode = currentNode.parentNode) {
      if (Util.checkHasClass(currentNode, DRAWER_CLASS)) {
        ...
        !Util.checkHasClass(currentNode, lux.classes.CLOSE)
      ) {
  ...
  hide() {
    ...
    const secondLevelDrawer = this._drawers.filter(drawer => Util.hasClass(drawer, lux.classes.ACTIVE))[0];
    ...
    if (secondLevelDrawer) {
      ...
      setTimeout(
        function() {
          Util.removeClass(secondLevelDrawer, lux.classes.ACTIVE);
          Util.removeClass(firstLevelDrawerContent, SECOND_LEVEL_SHOWN_CLASS);
        },
        DRAWER_ANIMATION_DURATION
      );
  ...
  drawerMenuItemRemoveUnselected() {
    ...
    if (drawerActiveMenuItem && Util.checkHasClass(drawerActiveMenuItem, lux.classes.UNSELECTED)) {
      Util.checkRemoveClass(drawerActiveMenuItem, lux.classes.UNSELECTED);
    }
  }

  _toggleDrawerItemList(drawerMenuItem) {
    const activeDrawerMenuItem = this._mobileNavElement.querySelector(`.${MOBILENAV_DRAWERS_CLASS} .${DRAWER_CLASS} .${DRAWER_CONTENT_CLASS} .${DRAWER_NAV_LIST} li.${lux.classes.ACTIVE}`);
    const drawerMenuItemList = drawerMenuItem.querySelector(`.${DRAWER_ITEM_LIST_CLASS}`);
    ...
    if (drawerMenuItem !== activeDrawerMenuItem &&
      !Util.hasClass(drawerMenuItem, lux.classes.EXPANDED)) {
      Util.checkAddClass(activeDrawerMenuItem, lux.classes.UNSELECTED);
    } else {
      this.drawerMenuItemRemoveUnselected();
    }
    ...
    if (!Util.checkHasClass(drawerMenuItem, DRAWER_ITEM_LIST_EXPANDED)) {
      this._menuItemAnimation = Util.threeStepsAnimation(
        function() {
          calculateHeight();
          Util.checkAddClass(drawerMenuItemList, lux.classes.TRANSITIONING);
          ...
          Util.checkRemoveClass(drawerMenuItemList, lux.classes.TRANSITIONING);
  ...
  resetActiveDrawerMenuItem() {
    ...
    if (drawerExpandedMenuItem) {
      ...
      if (drawerExpandedMenuItemList &&
        !drawerExpandedMenuItemList.querySelector(`ul li a.${lux.classes.ACTIVE}`)) {
        drawerExpandedMenuItem.querySelector(`.${DRAWER_ITEM_LIST_CLASS}`).style.removeProperty('height');
        Util.checkRemoveClass(drawerExpandedMenuItem, lux.classes.EXPANDED);
      }
    }
    ...
    if (currentlyActiveDrawerItemSubtab) {
      ...
      Util.checkAddClass(drawerMenuItemToActivate, lux.classes.ACTIVE);
      Util.checkAddClass(drawerMenuItemToActivate, lux.classes.EXPANDED);
    }
  }
