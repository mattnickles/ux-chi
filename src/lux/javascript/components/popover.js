...
const CLASS_POPOVER = 'lux-popover';
const TRANSITION_DURATION = 200;
const EVENTS = {
  SHOW: 'lux.popover.show',
  HIDE: 'lux.popover.hide'
};
...
class Popover extends Component {
  ...
  show(force) {
    ...
    if (!this._config.animate) {
      Util.addClass(this._popoverElem, lux.classes.ACTIVE);
    ...
    Util.addClass(self._popoverElem, lux.classes.TRANSITIONING);
  ...
  hide(force) {
    ...
    if (!this._config.animate) {
      Util.removeClass(this._popoverElem, lux.classes.ACTIVE);
    ...
    Util.threeStepsAnimation(
      ...
      function() {
        self._popoverElem.style.transform = self._preAnimationTransformStyle;
        Util.removeClass(self._popoverElem, lux.classes.ACTIVE);
      },
      function() {
        Util.removeClass(self._popoverElem, lux.classes.TRANSITIONING);
  ...
  _configurePopoverClasses() {
    ...
    if (this._config.animate) {
      Util.addClass(this._popoverElem, lux.classes.ANIMATED);
    }
  }
  ...
  _configurePopoverContent() {
    ...
    if (this._config.arrow) {
      ...
      arrow.className = 'lux-popover__arrow';
  ...
  _configurePopoverIdAria() {
    this._popoverElem.id =
      this._popoverElem.id ||
      'lux-' + COMPONENT_TYPE + '-' + this.componentCounterNo;
    this._config.parent.setAttribute('aria-describedby', this._popoverElem.id);
    this._popoverElem.setAttribute('aria-hidden', 'true');
    this._popoverElem.setAttribute('role', 'dialog');
    if (this._popoverElem.querySelector('.lux-popover__title')) {
      this._popoverElem.setAttribute(
        'aria-label',
        this._popoverElem.querySelector('.lux-popover__title').innerHTML
      );
    }
